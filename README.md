...
빌드가 끝날 때마다 `tools/check-weak-stubs.py`가 링커 맵을 읽습니다. 실패하면 거기서 멈춥니다.

`led_breath.c`는 ZMK 함수 몇 개를 `__weak`으로 선언해 둡니다. 그 덕에 한쪽 반쪽에만 있는 함수를 참조해도 링크가 되는데, **ZMK가 그 함수의 이름을 바꿔도 똑같이 링크가 됩니다.** 진짜 함수가 사라지고 빈 껍데기가 대신 들어가는데 빌드는 성공하고, 증상은 상태 LED가 엉뚱하게 켜지는 것뿐입니다. 오류도 경고도 안 납니다.

껍데기가 실제로 쓰였는지는 링커 맵에 남으므로, 그걸 매번 읽습니다. ZMK 버전을 올릴 때 제일 먼저 봐야 할 곳입니다.
